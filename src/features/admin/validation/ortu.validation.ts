import { z } from "zod";

const MAX_FILE_SIZE = 1000000; // 1 MB
const ACCEPTED_IMAGE_TYPES = ["image/jpeg", "image/jpg", "image/png"];

export const ortuRegisterSchema = z.object({
  email: z.string().email({message: "Email tidak valid" }),
  password: z.string().min(8, {message: "Kata sandi minimal 8 karakter" }),
  nama: z.string().min(3, {message: "Nama minimal 3 karakter" }),
  nomorTelepon: z.string().min(10, {message: "Nomor telepon tidak valid" }),
  alamat: z.string().min(3, {message: "Alamat minimal 3 karakter" }),
  jenisKelamin: z.enum(["L", "P"]).refine((val) => val === "L" || val === "P", {message: "Jenis kelamin harus di isi" }),
  tipe: z.enum(["Ayah", "Ibu", "Wali"]).refine((val) => val === "Ayah" || val === "Ibu" || val === "Wali", {message: "Tipe Orang Tua harus di isi" }),
  foto: z.custom<File | undefined>().optional().refine((file) => {
    if (!file) {
      return true;
    }
    // Validasi size file
    return file.size <= MAX_FILE_SIZE;
  }, `Ukuran foto tidak boleh lebih dari 1MB.`)
      .refine((file) => {
        if (!file) {
          return true;
        }
        // Validasi type file
        return ACCEPTED_IMAGE_TYPES.includes(file.type);
      }, "Hanya file .jpg, .jpeg, dan .png yang diizinkan."),
});

export const ortuUpdateSchemaAdmin = ortuRegisterSchema.omit({ email: true, password: true, });
