...
const ManageOrtu = () => {
  ...
  const { data, isLoading, isError, isFetching } = useOrtuList(currentPage, itemsPerPage, searchFilter);
  const deleteMutation = useDeleteOrtuMutation();
  ...
  const handleDeleteClick = (ortuId: number, ortuName: string) => {
    setSelectedOrtu(ortuName);
    setOrtuToDelete(ortuId);
    setIsDeleteDialogOpen(true);
  };

  const confirmDelete = () => {
    if (ortuToDelete !== null) {
      deleteMutation.mutate(ortuToDelete);
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h2 className="text-3xl font-bold text-violet-600">Manage Orang Tua</h2>
          <p className="text-muted-foreground">Manage Orang Tua data</p>
        </div>
        <Card>
          <CardHeader>
            <CardTitle className="text-violet-600">Daftar Orang Tua</CardTitle>
                  ...
                  <TableBody>
                    {isLoading ? (
                      ...
                      <TableRow>
                        ...
                        >
                          {/* Error: {error?.message} */}
                          Gagal memuat data orang tua. Coba lagi.
                        </TableCell>
                      </TableRow>
                    ) : ortuList.length > 0 ? (
                      ortuList.map((ortu, index) => (
                        <TableRow key={ortu.id}>
                          ...
                          <TableCell className="font-medium">
                            {ortu.nama.length > 15 ? ortu.nama.substring(0, 10) + "..." : ortu.nama}
                          </TableCell>
                          <TableCell className="hidden md:table-cell">{ortu.user.email}</TableCell>
                          <TableCell className="hidden md:table-cell">{ortu.nomorHp}</TableCell>
                          <TableCell className="hidden md:table-cell">{ortu.alamat}</TableCell>
                          <TableCell className="text-center">
                            <div className="flex gap-2 justify-center">
                              <Link to={`/admin/orangtua/detail/${ortu.id}`}>
                                <Button size="sm" variant="outline" className="bg-green-500 hover:bg-green-600 text-white hover:text-white">
                                  <Eye className="h-4 w-4" />
                                </Button>
                              </Link>
                              <Link to={`/admin/orangtua/edit/${ortu.id}`}>
                                <Button size="sm" variant="outline" className="bg-blue-500 hover:bg-blue-600 text-white hover:text-white">
                              ...
                              <Button
                                size="sm"
                                variant="destructive"
                                onClick={() => handleDeleteClick(ortu.id, ortu.nama)}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              menghapus data Orang Tua (<span className="font-bold text-black">{selectedOrtu}</span>).
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
