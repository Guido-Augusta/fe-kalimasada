...
import { ustadRegisterSchema } from "../../validation/ustad.validation";
import { useAddUstadzMutation } from "../../hooks/useUstadzData";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";
import { useNavigate } from "react-router-dom";
import { generateRandomPassword } from "@/utils/randomPassword";

type FormData = z.infer<typeof ustadRegisterSchema>;

const AddUstadz = () => {
  const [selectedImage, setSelectedImage] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement | null>(null);
  const addMutation = useAddUstadzMutation();
  const navigate = useNavigate();
  const [showPassword, setShowPassword] = useState(false);

  const form = useForm<FormData>({
    resolver: zodResolver(ustadRegisterSchema),
    defaultValues: {
      email: "",
      password: "",
      nama: "",
  ...
  const onSubmit = (data: FormData) => {
    ...
    formData.append("waliKelasTahap", data.waliKelasTahap || "");
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card>
            <CardHeader className="text-center">
              ...
              <CardDescription>
                Lengkapi form berikut untuk mendaftarkan ustadz baru
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)}>
                  <div className="flex flex-col md:flex-row md:space-x-8">
                    ...
                    <div className="md:w-1/2 md:space-y-6 mt-6 space-y-4 md:mt-0">
                      <FormField
                        control={form.control}
                        name="alamat"
                        render={({ field }) => (
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            <FormLabel>Penaggung Jawab Kelas (opsional)</FormLabel>
                            <FormControl>
                              <Select
                                onValueChange={field.onChange}
                              ...
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="Pilih Tingkatan Ajar" />
                                </SelectTrigger>
                                <SelectContent>
