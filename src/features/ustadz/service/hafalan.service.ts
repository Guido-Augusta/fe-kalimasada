...
import type { DeleteRiwayatHafalanResponse, HafalanData, HafalanProgressApiData, RiwayatDetailResponse, RiwayatHafalanResponse, RiwayatHafalanTerakhirResponse } from "../types/hafalan.type";
...
export const deleteRiwayatHafalan = async (data: { santriId: number; surahId: number; tanggal: string; status: "TambahHafalan" | "Murajaah" }): Promise<DeleteRiwayatHafalanResponse> => {
  const headers = getAuthHeaders(true);
...
export const fetchRiwayatHafalan = async (santriId: string, page: number, status: "TambahHafalan" | "Murajaah"): Promise<RiwayatHafalanResponse> => {
  const headers = getAuthHeaders(false);
...
export const fetchRiwayatDetail = async (santriId: string, surahId: string, tanggal: string, status: string): Promise<RiwayatDetailResponse> => {
  const headers = getAuthHeaders(false);
  ...
  if (!response.ok) {
    const errorData = await response.json();
    throw new Error(errorData.message || "Gagal mengambil detail riwayat hafalan.");
  }
  return response.json();
};
