...
import { useUpdateProfileUstadzMutation } from "../hooks/useEditProfileUstadz";
import { ustadEditProfileSchema } from "../validation/ustadEditValidation";
import useUser from "@/store/useUser";

type FormData = z.infer<typeof ustadEditProfileSchema>;

export default function UstadEditProfile() {
  const { user } = useUser()
  const navigate = useNavigate();
  
  const [previewImage, setPreviewImage] = useState<string | null>(null);

  const { data, isLoading: isFetchingData, isError, error: _error } = useUstadzDetail(user?.roleId as string);
  const updateMutation = useUpdateProfileUstadzMutation();

  const form = useForm<FormData>({
    resolver: zodResolver(ustadEditProfileSchema),
    values: data ? {
        nama: data.nama,
      ...
      <div className="container mx-auto max-w-4xl">
        <div className="flex items-center gap-4 mb-6">
          <Button 
            variant="outline" 
            size="sm" 
            // onClick={() => navigate("/ustadz/profile")}
            onClick={() => navigate(-1)}
