...
import { ortuEditProfileSchema } from "../validation/ortuEditValidation";
import { useUpdateProfileOrtuMutation } from "../hooks/useEditProfileOrtu";
import { useOrtuDetail } from "@/features/admin/hooks/useOrtuData";
import useUser from "@/store/useUser";

type FormData = z.infer<typeof ortuEditProfileSchema>;

export default function OrtuEditProfile() {
  const { user } = useUser(); 
  const navigate = useNavigate();
  
  const [previewImage, setPreviewImage] = useState<string | null>(null);

  const { data, isLoading: isFetchingData, isError, error: _error } = useOrtuDetail(user?.roleId as string);
  const updateMutation = useUpdateProfileOrtuMutation();

  const form = useForm<FormData>({
    resolver: zodResolver(ortuEditProfileSchema),
    values: data ? {
        nama: data.nama,
  ...
  const onSubmit = (formData: FormData) => {
    if (user?.roleId) {
      ...
      updateMutation.mutate({ id: user?.roleId as string, formData: putFormData });
    }
  };
