...
import { useFetchJuzOrtu } from '../hooks/useFetchJuzOrtu';
...
export default function OrtuBacaJuz() {
  const navigate = useNavigate();
  const { idSantri } = useParams<{ idSantri: string }>();
  const { juzData, loading, error } = useFetchJuzOrtu();
    ...
    <OrtuLayout>
      <div className="container mx-auto">
        ...
        <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
          <Link to={`/ortu/hafalan/${idSantri}/juz/${prevJuz}`}>
            <Button
              variant="ghost"
          ...
          <Link to={`/ortu/hafalan/${idSantri}/juz/${nextJuz}`}>
            <Button
              variant="ghost"
              className="flex items-center text-blue-500 hover:text-blue-700"
            >
