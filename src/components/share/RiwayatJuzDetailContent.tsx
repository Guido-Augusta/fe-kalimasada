...
export default function RiwayatJuzDetailContent({ santriId, juzId, tanggal, status }: RiwayatJuzDetailContentProps) {
  const { data, isLoading, isError } = useRiwayatJuzDetail(santriId, juzId, tanggal, status);
    ...
    <div className="container mx-auto p-2 md:p-6 lg:p-8">
        ...
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-center">
            ...
            <div className="flex md:flex-row flex-col items-center gap-2 md:justify-center justify-between mt-2">
              <p className="text-base text-white order-2 md:order-1">{formatTanggalIndo(riwayatJuzDetail.tanggal)}</p>
              <Badge className={getBadgeStatus(riwayatJuzDetail.status) + " order-1 md:order-2"}>
      ...
      <div className="grid grid-cols-3 gap-2 md:gap-4 text-center sm:grid-cols-3 my-3">
        ...
        <div className="border bg-amber-100 border-violet-600/90 py-4 px-2 rounded-xl">
          ...
          <p className="text-wrap ">{riwayatJuzDetail.rangeHalaman.awal} - {riwayatJuzDetail.rangeHalaman.akhir}</p>
        </div>
        <div className="border bg-amber-100 border-violet-600/90 py-4 px-2 rounded-xl">
